<?php
    class Destino {
        ...
        public static function obtenerDestinosOrdenados(){
            $objAccesoDatos = AccesoDatos::obtenerInstancia();
            $consulta = $objAccesoDatos->prepararConsulta("SELECT d.idDestino, d.nombre, COUNT(m.destino) AS cantidadDeMensajes FROM destino AS d, mensaje AS m WHERE d.idDestino = m.destino GROUP BY m.destino ORDER BY cantidadDeMensajes");
            $consulta->execute();
